			...
			match &msg.update.kind {
					...
					if let MessageKind::Text { data, entities } = &msg.kind {
						...
						if command.is_some() {
							self.command_handler
								.send(TelegramCommand {
									command: data.clone(),
									chat:    msg.chat.clone(),
								})
								.await
								.expect("The CommandHandler actor should always be alive");
						}
					}
				}
				UpdateKind::CallbackQuery(cb) => {
					...
					self.callback_router
						.send(RouteCallback { query: cb.clone() })
						.await
						.expect("The CallbackRouter actor should always be alive");
				}
				_ => {}
			}
		}
