...
use stage::{
	actors::{Actor, ActorError, BuildActorOp},
	system::{ActorSystemBuilder, ActorSystemConfig, SysHandle},
	traits::ActorFn,
};
...
async fn main() {
	...
	let telegram_sender = create_actor(
		&handle,
		TelegramSender::new(api.clone()),
		TelegramSender::handle,
	);
	let callback_router = create_actor(
		&handle,
		CallbackRouter::new(telegram_sender.clone()),
		CallbackRouter::handle,
	);
	let scheduler = create_actor(
		&handle,
		Scheduler::new(
			database.clone(),
	...
	let command_handler = create_actor(
		&handle,
		CommandHandler::new(
			telegram_sender.clone(),
			callback_router.clone(),
